                                     ...
                                     float beta)
{
    ...
    const int bx = blockIdx.x * blockDim.x * kThreadM;
    const int by = blockIdx.y * blockDim.y * kThreadN;
    ...
    float4 regB[2][kThreadM / 4]; // hopefully, these should reside in register.
    ...
    int rowA = tid >> 1, rowB = tid >> 5, colA = (tid & 1) << 2, colB = tid & 31;
    int warpId = tid >> 5, warpBaseId = tid & 31;
...
#pragma unroll
        for (int ii = 0; ii < kBlockK; ii++)
        {
            if (ii != 7)
            {
                regB[(ii + 1) % 2][0] = *reinterpret_cast<float4 *>(&subB[colC + kBlockN * (ii + 1)]);
                regB[(ii + 1) % 2][1] = *reinterpret_cast<float4 *>(&subB[colC + 32 + kBlockN * (ii + 1)]);

                regA[(ii + 1) % 2][0] = *reinterpret_cast<float4 *>(&subA[rowC + (ii + 1) * ldSubA]);
                regA[(ii + 1) % 2][1] = *reinterpret_cast<float4 *>(&subA[(rowC + 16) + (ii + 1) * ldSubA]);
            }
...
int main(int argc, char * argv[])
{
    ...
    constexpr bool kTestGemmDoubleBuffer = true;
    ...
    // Double buffer.
    if constexpr (kTestGemmDoubleBuffer)
    {
        if constexpr (1 < kDup)
        {
            gemmDoubleBuffer<kBlockSize, kBlockM, kBlockN, kBlockK><<<grid, block>>>(
                    thrust::raw_pointer_cast(d_a.data()),
                    thrust::raw_pointer_cast(d_b.data()),
                    thrust::raw_pointer_cast(d_c.data()),
                    m,
