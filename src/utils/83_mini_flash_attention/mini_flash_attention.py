...
def flash_attn_fwd(Q: torch.Tensor,
    ...
    for i in range(Tr):
        ...
        m: torch.Tensor = torch.full((Br,), fill_value=-torch.inf, dtype=dtype, device=device)  # (Br,)
        ...
        for j in range(Tc):
            ...
            m = torch.maximum(m_old, torch.max(S, dim=-1)[0])  # (Br,)

            P: torch.Tensor = torch.exp(S - m.unsqueeze(-1))  # (Br, Bc,)

            l = torch.exp(m_old - m) * l + torch.sum(P, dim=-1)  # (Br,)
            Oi = torch.diag(torch.exp(m_old - m)) @ Oi + P @ Vj  # (Br, d,)

        O[i * Br:(i + 1) * Br, :] = torch.inverse(torch.diag(l)) @ Oi  # (Br, d,)
...
def flash_attn_bwd(Q: torch.Tensor,
    ...
    assert Q.ndim == K.ndim == V.ndim == O.ndim == dO.ndim == L.ndim + 1 == 2
    ...
    dQ: torch.Tensor = torch.zeros((n, d,), dtype=dtype, device=device)  # (n, d,)
    dK: torch.Tensor = torch.zeros((n, d,), dtype=dtype, device=device)  # (n, d,)
    dV: torch.Tensor = torch.zeros((n, d,), dtype=dtype, device=device)  # (n, d,)
    ...
    for j in range(Tc):
        ...
        for i in range(Tr):
            Qi: torch.Tensor = Q[i * Br:(i + 1) * Br, :] * softmax_scale  # (Br, d,)
            Oi: torch.Tensor = O[i * Br:(i + 1) * Br, :]  # (Br, d,)
            dOi: torch.Tensor = dO[i * Br:(i + 1) * Br, :]  # (Br, d,)
            Li: torch.Tensor = L[i * Br:(i + 1) * Br]  # (Br,)

            Di: torch.Tensor = torch.sum(dOi * Oi, dim=-1)  # (Br,)
            S: torch.Tensor = Qi @ Kj.transpose(-2, -1)  # (Br, Bc,)
...
def main() -> None:
    ...
    QQ: torch.Tensor = torch.rand(batch_size, num_heads, seqlen, embed_dim).to(dtype=dtype, device=device) * 7
    KK: torch.Tensor = torch.rand(batch_size, num_heads, seqlen, embed_dim).to(dtype=dtype, device=device) * 4
    VV: torch.Tensor = torch.rand(batch_size, num_heads, seqlen, embed_dim).to(dtype=dtype, device=device) * 9
