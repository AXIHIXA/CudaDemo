                                    ...
                                    const int num_elements, T * scale, T * zero_point)
{
    ...
    for (int cid = 0; cid < channel; cid++)
    {
        ...
        T channel_min = *std::min_element(in_ptr + start, in_ptr +
                                                          end);// note: cannot use [] which get a float, must use + to get pointer
                                           ...
                                           const int channel, const int HW)
{
    ...
    while (gid < nums)
    {
        ...
        if (gid == 328)
        {
            printf("328 in_ptr is %f, out is %f\n", in_ptr[gid], out);
        }
        if (gid == 1587)
        {
            printf("1587 in_ptr is %f, out is %f\n", in_ptr[gid], out);
        }
        out = out > upper_bound ? upper_bound : out;
        out = out < lower_bound ? lower_bound : out;
        out_ptr[gid] = out;
